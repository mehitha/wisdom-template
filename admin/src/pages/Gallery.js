...
function Gallery() {
  ...
  const handleUpload = async () => {
    ...
    setLoading(true);
    try {
      await axios.post(`${API_URL}/api/albums/${uploadAlbumId}/images`, formData, {
        headers: { "Content-Type": "multipart/form-data" },
      });
      alert("✅ Image Successfully Uploaded!");
      setSelectedFile(null);
  ...
  const handleAddAlbum = async () => {
    if (!newAlbumName.trim()) return alert("Enter album name!");
    try {
      const res = await axios.post(`${API_URL}/api/albums`, {
        name: newAlbumName,
      });
      setAlbums((prev) => [{ ...res.data, images: [] }, ...prev]);
      setNewAlbumName("");
  ...
  const handleDeleteImage = async (albumId, imagePath) => {
    ...
    const filename = imagePath.split('/').pop();
    try {
      await axios.delete(`${API_URL}/api/albums/${albumId}/images/${filename}`);
      fetchAlbums();
      alert("✅ Image deleted!");
    } catch (error) {
      console.error("Delete error:", error);
